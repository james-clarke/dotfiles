...
if [ "$have" != "$want" ]; then
	...
	*) sum= ;;
	...
	if [ "$codename" = trixie ] && [ -n "$sum" ] &&
		...
		echo "$sum  $tmp/$deb" | sha256sum -c --quiet || {
			echo "ghostty checksum mismatch"
			exit 1
		}
...
if ! command -v claude >/dev/null; then
	...
	gpg --show-keys --with-colons /etc/apt/keyrings/claude-code.asc | grep -q "^fpr:.*:$CLAUDE_KEY_FPR:" ||
		{
			echo "claude-code.asc fingerprint mismatch"
			exit 1
		}
...
step "google chrome"
if [ "$(dpkg --print-architecture)" = amd64 ] && ! command -v google-chrome >/dev/null; then
	curl -fsSL https://dl.google.com/linux/linux_signing_key.pub -o "$tmp/google.asc"
	gpg --show-keys --with-colons "$tmp/google.asc" | grep -q "^fpr:.*:$CHROME_KEY_FPR:" ||
		{
			echo "google key fingerprint mismatch"
			exit 1
		}
	gpg --dearmor <"$tmp/google.asc" | sudo tee /usr/share/keyrings/google-chrome.gpg >/dev/null
